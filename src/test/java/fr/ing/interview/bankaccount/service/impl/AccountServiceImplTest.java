...
public  class AccountServiceImplTest {
    ...
    @Test
    public void givenDepositAmountequalLimitThenDoNotSaveBankAccount() {
        bankAccountService.deposit(BANK_ACCOUNT_ID, DEPOSIT_AMOUNT_LIMIT);
        verify(accountRepository, never()).save(eq(bankAccount));
        assertEquals(0,bankAccount.getBalance().compareTo(BigDecimal.valueOf(1000.00)));
    }
    ...
    public void givenDepositAmountLessThanLimitThenDoNotSaveBankAccount() {
        bankAccountService.deposit(BANK_ACCOUNT_ID, 00.00);
        verify(accountRepository, never()).save(eq(bankAccount));
        assertEquals(0,bankAccount.getBalance().compareTo(BigDecimal.valueOf(1000.00)));
    }
